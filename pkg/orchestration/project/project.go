...
import (
	...
	"github.com/apigear-io/cli/pkg/foundation/vfs"
)
...
func OpenProject(source string) (*ProjectInfo, error) {
	...
	if _, err := os.Stat(foundation.Join(source, "apigear")); err != nil {
		return nil, err
	}
...
func InitProject(d string) (*ProjectInfo, error) {
	...
	if err := os.Mkdir(foundation.Join(d, "apigear"), 0755); err != nil {
		if !os.IsExist(err) {
			return nil, err
		}
	}
	// write demo module
	target := foundation.Join(d, "apigear", "demo.module.yaml")
	if err := writeDemo(target, vfs.DemoModuleYaml); err != nil {
		log.Debug().Msgf("write demo module: %s", err)
	}
	target = foundation.Join(d, "apigear", "demo.module.idl")
	if err := writeDemo(target, vfs.DemoModuleIdl); err != nil {
		log.Debug().Msgf("write demo module: %s", err)
	}
	// write demo solution
	target = foundation.Join(d, "apigear", "demo.solution.yaml")
	...
	target = foundation.Join(d, "apigear", "demo.sim.js")
...
func RecentProjectInfos() []*ProjectInfo {
	...
	for _, d := range config.RecentEntries() {
		...
		if err != nil {
			...
			err = config.RemoveRecentEntry(d)
			if err != nil {
...
func AddDocument(prjDir string, docType string, name string) (string, error) {
	target := foundation.Join(prjDir, "apigear", MakeDocumentName(docType, name))
	var err error
	switch docType {
	case "module":
