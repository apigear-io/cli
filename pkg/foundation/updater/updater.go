...
func (u *Updater) Check(ctx context.Context) (*selfupdate.Release, error) {
	logging.Info().Msgf("check for updates: %s", u.repo)
	...
	logging.Info().Msgf("latest release: %s", latest.Version())
	if !latest.GreaterThan(u.version) {
		logging.Info().Msgf("current version %s is the latest", u.version)
		return nil, nil
	}
	logging.Info().Msgf("new version %s is available", latest.Version())
	return latest, nil
}
