...
func NewConfig(cfgDir string) (*viper.Viper, error) {
	...
	cfgFile := foundation.Join(cfgDir, "config.json")
	nv.AddConfigPath(cfgDir)
	nv.SetConfigType("json")
	nv.SetConfigName("config")

	if !foundation.IsFile(cfgFile) {
		err := foundation.MakeDir(cfgDir)
		if err != nil {
			return nil, fmt.Errorf("failed to create config dir: %w", err)
		}
		err = foundation.WriteFile(cfgFile, []byte("{}"))
		if err != nil {
