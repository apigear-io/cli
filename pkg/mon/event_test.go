...
func TestEventSubject(t *testing.T) {
	t.Run("returns mon.source format", func(t *testing.T) {
		event := &Event{
			Source: "device123",
		}
		assert.Equal(t, "mon.device123", event.Subject())
	})
	...
	t.Run("handles empty source", func(t *testing.T) {
		...
		assert.Equal(t, "mon.", event.Subject())
	})
	...
	t.Run("handles source with special characters", func(t *testing.T) {
		...
		assert.Equal(t, "mon.device-123_test", event.Subject())
	})
}
