...
// Events are still emitted to local hooks via monitoring.Emitter.FireHook()
//
...
func MonitorRequestHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		logging.Debug().Msgf("handle monitor request %s", source)
		if source == "" {
			logging.Error().Msg("source id is required")
		...
		if err != nil {
			logging.Error().Msgf("decode event: %v", err)
...
func HandleMonitorRequest(w http.ResponseWriter, r *http.Request) {
	logging.Debug().Msg("handle monitor request")
	source := chi.URLParam(r, "source")
	if source == "" {
		logging.Error().Msg("source id is required")
		http.Error(w, "source id is required", http.StatusBadRequest)
