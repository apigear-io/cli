...
func NewManager() *NetworkManager {
	logging.Debug().Msg("net.NewManager")
	return &NetworkManager{}
}

func (s *NetworkManager) Start(opts *Options) error {
	s.opts = opts
	logging.Debug().Msg("start network manager")
	if !s.opts.HttpDisabled {
		err := s.StartHTTP(s.opts.HttpAddr)
		if err != nil {
			logging.Error().Err(err).Msg("failed to start http server")
			return err
		}
	}
	if !s.opts.MonitorDisabled {
		err := s.EnableMonitor()
		if err != nil {
			logging.Error().Err(err).Msg("failed to enable monitor")
			return err
		}
	}
	return nil
...
func (s *NetworkManager) Wait(ctx context.Context) error {
	logging.Info().Msg("services running...")
	sig := make(chan os.Signal, 1)
	signal.Notify(sig, syscall.SIGINT, syscall.SIGTERM)
	defer func() {
		err := s.Stop()
		if err != nil {
			logging.Error().Err(err).Msg("failed to stop services")
		}
		logging.Info().Msg("services stopped")
	}()
	select {
	case <-ctx.Done():
		return ctx.Err()
...
func (s *NetworkManager) Stop() error {
	logging.Info().Msg("stop network manager")
	err := s.StopHTTP()
	if err != nil {
		return err
	}
...
func (s *NetworkManager) StartHTTP(addr string) error {
	if s.httpServer != nil {
		logging.Info().Msg("stop running http server")
		s.httpServer.Stop()
	}
	logging.Info().Msg("start http server")
	s.httpServer = NewHTTPServer(&HttpServerOptions{Addr: addr})
	err := s.httpServer.Start()
	if err != nil {
		logging.Error().Err(err).Msg("failed to start http server")
	}
	logging.Info().Msgf("http server started at http://%s", addr)
	return err
}

func (s *NetworkManager) StopHTTP() error {
	logging.Info().Msg("stop http server")
	if s.httpServer != nil {
		s.httpServer.Stop()
	}
	return nil
...
func (s *NetworkManager) GetMonitorAddress() (string, error) {
	logging.Info().Msg("get monitor address")
	if s.httpServer == nil {
		return "", fmt.Errorf("http server not started")
	}
...
func (s *NetworkManager) GetSimulationAddress() (string, error) {
	logging.Info().Msg("get simulation address")
	if s.httpServer == nil {
		return "", fmt.Errorf("http server not started")
	}
...
func (s *NetworkManager) MonitorEmitter() *foundation.Hook[monitoring.Event] {
	return &monitoring.Emitter
}
