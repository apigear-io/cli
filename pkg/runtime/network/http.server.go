...
func (s *HTTPServer) Start() error {
	if s.server != nil {
		logging.Info().Msgf("http server already started at %s", s.server.Addr)
		return nil
	}
	if s.opts.Addr == "" {
...
func (s *HTTPServer) Restart(ctx context.Context, addr string) error {
	...
	logging.Debug().Msgf("restart http server at %s", addr)
	s.server.RegisterOnShutdown(func() {
		logging.Info().Msgf("shutdown http server at %s", addr)
	})
...
func (s *HTTPServer) Stop() {
	...
	logging.Debug().Msgf("stop http server at %s", s.server.Addr)
