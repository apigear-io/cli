...
func TestParamSymbols(t *testing.T) {
	...
	table := []struct {
		...
		{"test", "Test2", "propInterface", "const TScriptInterface<ITestInterface1>& PropInterface"},
		...
		{"test", "Test2", "propInterfaceArray", "const TArray<TScriptInterface<ITestInterface1>>& PropInterfaceArray"},
	}
	syss := loadTestSystems(t)
	for _, sys := range syss {
		for _, tt := range table {
