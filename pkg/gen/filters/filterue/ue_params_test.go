...
func TestParams(t *testing.T) {
	...
	table := []struct {
		...
		{"test", "Test3", "op_Bool", "bool bParamBool"},
		{"test", "Test3", "op_bool", "bool bParamBool"},
		{"test", "Test3", "op_1", "bool bParam1"},
	}
...
func TestParamsSymbols(t *testing.T) {
	...
	table := []struct {
		...
		{"test", "Test4", "opInterface", "const TScriptInterface<ITestInterface1>& Param1"},
		{"test", "Test4", "opEnumArray", "const TArray<ETestEnum1>& Param1"},
		{"test", "Test4", "opStructArray", "const TArray<FTestStruct1>& Param1"},
		{"test", "Test4", "opInterfaceArray", "const TArray<TScriptInterface<ITestInterface1>>& Param1"},
	}
	syss := loadTestSystems(t)
	for _, sys := range syss {
		for _, tt := range table {
...
func TestParamsMultiple(t *testing.T) {
	...
	table := []struct {
		...
		{"test", "Test5", "opInterfaceInterface", "const TScriptInterface<ITestInterface1>& Param1, const TScriptInterface<ITestInterface1>& Param2"},
	}
	syss := loadTestSystems(t)
	for _, sys := range syss {
		for _, tt := range table {
