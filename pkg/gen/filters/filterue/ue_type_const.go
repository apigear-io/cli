...
func ToConstTypeString(prefix string, schema *model.Schema) (string, error) {
	...
	case model.TypeInterface:
		text = fmt.Sprintf("const TScriptInterface<%sI%s%s>&", prefix, moduleId, schema.Type)
	default:
	...
	if schema.IsArray {
		...
		case model.TypeInterface:
			text = fmt.Sprintf("const TArray<TScriptInterface<%sI%s%s>>&", prefix, moduleId, schema.Type)
		default:
			return "xxx", fmt.Errorf("ueConstType unknown schema %s", schema.Dump())
		}
	}
