...
func (r *registry) Load() error {
	src := foundation.Join(r.RegistryDir, "registry.json")
	if !foundation.IsFile(src) {
		// try to update
...
func (c *registry) Save() error {
	dst := foundation.Join(c.RegistryDir, "registry.json")
...
func (r *registry) ensureRegistry() error {
	if !foundation.IsDir(r.RegistryDir) {
		err := r.Reset()
		if err != nil {
...
func (r *registry) Reset() error {
	...
	err := foundation.RemoveDir(r.RegistryDir)
	if err != nil {
