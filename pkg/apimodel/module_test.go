...
func readSystem(t *testing.T) *System {
	var system System
	var aModule Module
	err := foundation.ReadDocument("./testdata/a.module.yaml", &aModule)
	assert.NoError(t, err)
	system.AddModule(&aModule)

	var bModule Module
	err = foundation.ReadDocument("./testdata/b.module.yaml", &bModule)
	assert.NoError(t, err)
...
func TestModuleYaml(t *testing.T) {
	var module Module
	err := foundation.ReadDocument("./testdata/module.yaml", &module)
	assert.NoError(t, err)
...
func TestModuleJson(t *testing.T) {
	var module Module
	err := foundation.ReadDocument("./testdata/module.json", &module)
	assert.NoError(t, err)
	assert.Equal(t, "Module01", module.Name)
	assert.Equal(t, "1.0.0", string(module.Version))
}
