...
func (l *SolutionTarget) GetOutputDir(rootDir string) string {
	return foundation.Join(rootDir, l.Output)
}
...
func (l *SolutionTarget) compute(doc *SolutionDoc) error {
	...
	tplDir := foundation.Join(doc.RootDir, l.Template)
	if foundation.IsDir(tplDir) {
		l.TemplateDir = tplDir
		l.TemplatesDir = foundation.Join(tplDir, "templates")
		l.RulesFile = foundation.Join(tplDir, "rules.yaml")
	} else {
		// try to find the template dir in the templates dir
		repoId, err := registry.GetOrInstallTemplateFromRepoID(l.Template)
		if err != nil {
			log.Err(err).Msgf("failed to get template %s", l.Template)
			return err
		}
		tplDir, err := registry.Cache.GetTemplateDir(repoId)
		if err != nil {
	...
	if len(l.expandedInputs) == 0 {
		expanded, err := foundation.ExpandInputs(doc.RootDir, l.Inputs...)
		if err != nil {
			return err
		}
...
func (l *SolutionTarget) computeImports() error {
	...
	for _, imp := range l.Imports {
		err := foundation.ReadDocument(imp, &l.MetaImports)
		if err != nil {
