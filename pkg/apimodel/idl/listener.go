...
func IsNil(v any) {
	...
	logging.Error().Msgf("isNil: %v should be nil", v)
}
...
func (o *ObjectApiListener) EnterModuleRule(c *parser.ModuleRuleContext) {
	...
	version := ""
	if c.GetVersion() == nil {
		logging.Info().Msgf("module %s has no version, setting to 1.0", name)
		version = "1.0"
	} else {
...
func (o *ObjectApiListener) EnterImportRule(c *parser.ImportRuleContext) {
	...
	version := ""
	if c.GetVersion() == nil {
		logging.Info().Msgf("import %s has no version, setting to 1.0", name)
		version = "1.0"
	} else {
		version = c.GetVersion().GetText()
	}
...
func (o *ObjectApiListener) parseMeta(node *apimodel.NamedNode, ctxs []parser.IMetaRuleContext) {
	...
	if len(tagLines) > 0 {
		...
		if err != nil {
			logging.Warn().Err(err).Msgf("failed to parse meta data in %s:%d-%d", file, ymlStart, ymlEnd)
		}
	}
}
