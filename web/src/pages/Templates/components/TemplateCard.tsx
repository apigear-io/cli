...
import { Card, Stack, Group, Text, Badge, Button, Progress, ActionIcon, Tooltip, Menu } from '@mantine/core';
...
export function TemplateCard({ template }: TemplateCardProps) {
  ...
  const handleInstall = async (version?: string) => {
    ...
    const versionToInstall = version || template.latest;
    ...
    try {
      ...
      notifications.show({
        title: 'Success',
        message: `Template ${template.name}${versionToInstall ? ` ${versionToInstall}` : ''} installed successfully`,
        color: 'green',
            ...
            {template.versions && template.versions.length > 1 && (
              <Menu shadow="md" width={200} position="bottom-end">
                <Menu.Target>
                  <Button
                    variant={hasUpdate ? 'filled' : 'light'}
                    disabled={isUpToDate || installMutation.isPending}
